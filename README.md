...
1.  **`TA-archived-data-ingestor`**: (Installed on Search Head) Contains the user-facing dashboard and the custom command that generates `inputs.conf`.
...
2.  **Dashboard Interaction (`TA-archived-data-ingestor` app on SH):**
...
3.  **`inputs.conf` Generation (`bringbackdata` command on SH):**
...
1.  **`TA-archived-data-ingestor` App:** Installed on your **Search Head**. This app contains:
    *   The dashboard.
...
2.  **Access Dashboard:** Open the "Archived Data Ingestion Tool" dashboard on your Search Head.
