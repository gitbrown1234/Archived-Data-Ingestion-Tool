...
class BringBackDataCommand(GeneratingCommand):
    ...
    def generate(self):
        ...
        inputs_conf_content.append("# To apply these changes, you MUST restart Splunk or reload the app containing this inputs.conf (e.g., via Splunk Web > Apps > Manage Apps > Reload).")
        inputs_conf_content.append("#")
        ...
        try:
            ...
            yield {
                '_time': time.time(),
                'log_level': 'WARNING',
                'message': f"ACTION REQUIRED: To activate monitoring, you MUST restart Splunk or reload the app '{os.path.basename(os.path.dirname(os.path.dirname(self.inputs_conf_path)))}' (via Splunk Web > Apps > Manage Apps > Reload).",
                '_raw': f"ACTION REQUIRED: To activate monitoring, you MUST restart Splunk or reload the app '{os.path.basename(os.path.dirname(os.path.dirname(self.inputs_conf_path)))}' (via Splunk Web > Apps > Manage Apps > Reload)."
            }
